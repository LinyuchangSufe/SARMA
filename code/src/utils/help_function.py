...
def update_X_lmbd(X_lmbd,lmbd_k,k,y,p,P):
    T, N = y.shape
    P = min(T-p, P)
    lmbd_power = _safe_pow(lmbd_k, np.arange(1, P)) # truncated
    for j in range(N):
...
def update_X_eta(X_eta,eta_k,k,y,p,P):
    ...
    gam = _clip_stable_scalar(eta_k[0], STABLE_EPS, STABLE_RADIUS)
    cos_series = _safe_pow(gam, indices) * np.cos(indices * eta_k[1])
    sin_series = _safe_pow(gam, indices) * np.sin(indices * eta_k[1])
    for j in range(N):
...
def loss_mle(G, Sigma_inv,y, X_AR, X_lmbd, X_eta, p, r, s):
    ...
    T, _ = y.shape
    epsilon = get_epsilon(G, y, X_AR, X_lmbd, X_eta, p, r, s)
    det_val = np.linalg.det(Sigma_inv)
    if (not np.isfinite(det_val)) or (det_val <= 0):
        return np.inf
    loss = -(T-p) * np.log(det_val) + np.trace(epsilon.T @ epsilon @ Sigma_inv)
    if not np.isfinite(loss):
        return np.inf
    return loss/(T-p)
...
def vec_jac_lmbd(lmbd_k,k, G, Sigma_inv, y,X_AR,X_lmbd,X_eta, p,r,s,P): 
    ...
    indices = np.arange(1, P)
    lmbd_power = _safe_pow(lmbd_k, indices-1)
    for j in range(N):
...
def vec_jac_eta(eta_k,k,G, Sigma_inv,y,X_AR,X_lmbd,X_eta,p,r,s,P):
    ...
    indices = np.arange(1, P)
    gam = _clip_stable_scalar(eta_k[0], STABLE_EPS, STABLE_RADIUS)
    cos_series = _safe_pow(gam, indices-1) * np.cos(indices * eta_k[1])
    sin_series = _safe_pow(gam, indices-1) * np.sin(indices * eta_k[1])
    for j in range(N):
    ...
    first_grad_phi = gam * X_eta_k[:,:,1] @ G[:,:,p+r+2*k].T - gam * X_eta_k[:,:,0] @ G[:,:,p+r+2*k+1].T
...
def vec_jac_hess_eta(eta_k,k,G, Sigma_inv,y,X_AR,X_lmbd,X_eta,p,r,s,P):
    ...
    indices = np.arange(1, P)
    gam = _clip_stable_scalar(eta_k[0], STABLE_EPS, STABLE_RADIUS)
    power_series = _safe_pow(gam, indices-1)
    ...
    first_grad_phi = gam * X_eta_k_1[:,:,1] @ G[:,:,p+r+2*k].T - gam * X_eta_k_1[:,:,0] @ G[:,:,p+r+2*k+1].T
    ...
    hess_pp = np.trace(first_grad_phi.T @ first_grad_phi @ Sigma_inv)/(T-p) + \
        np.mean(np.sum(gam * (X_eta_k_2[:,:,0,1] @ G[:,:,p+r+2*k].T  + X_eta_k_2[:,:,1,1] @ G[:,:,p+r+2*k+1].T ) * (epsilon @ Sigma_inv), axis = 1))
...
def _clip_stable_scalar(x, lo=-STABLE_RADIUS, hi=STABLE_RADIUS):
...
def _safe_pow(base, exponents):
    # Robustify against occasional optimizer excursions that can push |base| >= 1.
    base_clipped = _clip_stable_scalar(base, -STABLE_RADIUS, STABLE_RADIUS)
    return np.power(base_clipped, exponents)
...
def optimize_parameter(
    ...
    if warmup_method == 'adam':
        ...
        for t in range(1, warmup + 1):
            ...
            param_new = clip_to_bounds(param - step)
        ...
        def update_cache(param_val):
            param_arr = clip_to_bounds(param_val)
            if np.max(np.abs(param_arr - cache['val'])) > cache_eps:
                cache['val'] = param_arr
                update_design_matrix_fn(param_arr)
                cache['loss'] = loss_fn()
                cache['grad'] = vec_jac_fn(param_arr)
                if (not np.isfinite(cache['loss'])) or np.any(~np.isfinite(cache['grad'])):
                    cache['loss'] = np.inf
                    cache['grad'] = np.zeros_like(param_arr)
        ...
        param = clip_to_bounds(warmup_result.x if warmup_result is not None else param)
    ...
    if refine_method:
        def update_cache(param_val):
            ...
            if np.max(np.abs(param_arr - cache['val'])) > cache_eps:
                ...
                if (
                    (not np.isfinite(cache['loss']))
                    or np.any(~np.isfinite(cache['grad']))
                    or np.any(~np.isfinite(h))
                ):
                    cache['loss'] = np.inf
                    cache['grad'] = np.zeros_like(param_arr)
                    cache['hess'] = np.eye(param_arr.size)
                else:
        ...
        param = clip_to_bounds(res.x if res is not None else param)
        update_design_matrix_fn(param)
    return param
