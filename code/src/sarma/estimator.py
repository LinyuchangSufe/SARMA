    ...
    * PCA / other preprocessing should live in a pipeline module,
    ...
    def __init__(
        ...
        *,
        ...
        self.g_gd_steps = g_gd_steps
        self.g_gd_tol = g_gd_tol
    ...
    def SARMA_fitBCD_SARMA(self,
        ...
        result = BCD_SARMA(y,p,r,s,lmbd = lmbd, eta = eta, Sigma = Sigma, 
                           ...
                           g_gd_steps = self.g_gd_steps,
                           g_gd_tol = self.g_gd_tol)
    ...
    def fit(
        ...
        r: Optional[int] = None,
        s: Optional[int] = None,
        init_lmbd: Optional[np.ndarray] = None,
        init_eta: Optional[np.ndarray] = None,
        init_label: str = "custom",
        n_jobs_BIC: int = 1,
        ...
        if (p is None) or (r is None) or (s is None):
            p, r, s, lmbd_init, eta_init, bic_res = self._choose_orders_via_bic(
                y,
                n_jobs_BIC=n_jobs_BIC,
                bic_criterion=bic_criterion,
        ...
        if init_lmbd is not None or init_eta is not None:
            if r > 0:
                if init_lmbd is None:
                    raise ValueError("init_lmbd is required when r > 0 and custom initialization is used.")
                lmbd0 = np.asarray(init_lmbd, dtype=float).reshape(-1)
                if lmbd0.size != r:
                    raise ValueError(f"init_lmbd size {lmbd0.size} != r={r}")
            else:
                lmbd0 = np.zeros(0, dtype=float)

            if s > 0:
                if init_eta is None:
                    raise ValueError("init_eta is required when s > 0 and custom initialization is used.")
                eta0 = np.asarray(init_eta, dtype=float).reshape(-1, 2)
                if eta0.shape != (s, 2):
                    raise ValueError(f"init_eta shape {eta0.shape} != ({s}, 2)")
            else:
                eta0 = np.zeros((0, 2), dtype=float)

            if self.verbose:
                print(
                    f"[Estimator] Running BCD from {init_label} initials for "
                    f"(p={p}, r={r}, s={s}): lmbd={lmbd0}, eta={eta0}"
                )

            res_ls = self.SARMA_fitBCD_SARMA(
                y,
                p,
                r,
                s,
            ...
            res_mle = self.SARMA_fitBCD_SARMA(
                y,
                p,
                r,
                s,
                lmbd=np.asarray(res_ls["lmbd"], dtype=float).copy(),
                eta=np.asarray(res_ls["eta"], dtype=float).copy(),
                Sigma=None,
                esti_method="mle",
                P=self.P,
                n_iter=self.n_iter,
            ...
            init_metadata = {
                "initial_lmbd": lmbd0.copy(),
                "initial_eta": eta0.copy(),
                "profile_loss": np.nan,
                "profile_candidate_index": 1,
                "profile_candidate_count": 1,
                "init_label": init_label,
            }
            res_ls.update(init_metadata)
            res_mle.update(init_metadata)
        else:
