...
def BCD_LS(y,p,r,s,P,n_iter=500,lmbd=None,gamma=None,phi=None,G=None,lr_omega=1,stop_thres=1e-3,stop_method='SepEst',result_show=False,flag_maxiter=0):
    '''
    Input:
    ------------------------------------
    y: data process ---N*T
    ...
    for iter_no in range(n_iter):
        ...
        for k in range(r): 
            ...
            Loss_after_gamma = loss_vec(Y,X1,G,L_c,T)
        ...
        for k in range(s):
            ...
            grad_gamma,grad_phi,hess = vec_jac_hess_gamma_phi([gamma[k],phi[k]],k,G,L,Y,X1,X2,p,r,T)
            ...
            L_c = L.copy()
            L_d = L.copy()
            Loss_pre_eta = loss_vec(Y,X1,G,L,T)
            for j in range(10**(N)):
                gamma_c = max(min(0.99,gamma[k] - c*direct[0]),0.01) 
                phi_c = max(min(0.99*np.pi,phi[k] - c*direct[1] ),0.01*np.pi)
                ...
                gamma_d = max(min(0.95,gamma[k] - d_*direct[0]),0.05) 
                phi_d = max(min(0.95*np.pi,phi[k] - d_*direct[1] ),0.05*np.pi)
            ...
            # print(stepsize)
            gamma[k] = max(min(0.99,gamma[k] - stepsize*direct[0]),0.01)
            phi[k] = max(min(0.99*np.pi,(phi[k] - stepsize*direct[1])),0.01*np.pi)
            ...
            Loss_after_eta = loss_vec(Y,X1,G,L_c,T)
            ...
            # gamma[k] = max(min(0.95,temp),0.05)
            # # phi[k] = max(min(np.pi/2,phi[k]),-np.pi/2) # phi \in (-pi/2,pi/2)
            # phi[k] = max(min(0.95*np.pi,phi[k]),0.05*np.pi) # phi \in (0,pi)
            # L[p:,p+r+2*k] = np.einsum('i,i->i',np.power(gamma[k],power_series),np.cos(power_series*phi[k]))
            # L[p:,p+r+2*k+1] = np.einsum('i,i->i',np.power(gamma[k],power_series),np.sin(power_series*phi[k]))
        ...
        if (stop_method == 'SepEst') and \
            ...
            if result_show == True:
                ...
                f'Param: lmbd: {lmbd},\n gamma: {gamma},\n phi: {phi},\n G: {G}, \n Sigma: {Sigma}\n',
                "=============================================")
            result = pd.Series(dict(zip(['Loss_plot','A', 'lmbd', 'eta','G','Sigma','Loss','flag_maxiter','iter_no'],
                     [Loss_plot,A,lmbd,eta,G,Sigma,Loss,flag_maxiter,iter_no])))
            return result
...
if __name__ == "__main__" :
    ...
    a= BCD_LS(y,p,r,s,P,n_iter,lmbd=np.array(lmbd), gamma = np.array([gamma]), phi = np.array([phi]), G=G,stop_thres=1e-4,result_show=True)
    end_time = time.time()
    end_time - start_time
    
    fit_lmbd ,fit_eta ,fit_G = a[['lmbd','eta','G']]
    
    # a= BCD_MLE(y,p,r,s,P,n_iter,lmbd=np.array([-0.7]), gamma = np.array([0.7]), phi = np.array([np.pi/4]), G=G,result_show=True)
    # BCD_LS(y,p,r,s,P,result_show=False)
    import matplotlib.pyplot as plt
    plt.plot(a['Loss_plot'])
    plt.show()
    np.linalg.norm(a['G']-G)
    (a['G']-G)[:,:,0]
    np.concatenate
    np.array((G[:,:,0].ravel('C'),np.eye(N)))
    Theta = G[:,:,0]*lmbd
    Theta_hat = a['G'][:,:,0] * a['lmbd']
    a['G'][:,:,0]
    eta= np.vstack((gamma, phi))
    L = get_L(lmbd,eta,r,s,T,p) # L: T*d matrix
    L_a = get_L(fit_lmbd,fit_eta,r,s,T,p) # L: T*d matrix
    loss_vec(Y,X1,fit_G,L_a,T)
    import statsmodels.api as sm
    mod = sm.tsa.VARMAX(y.T, order=(0,1), error_cov_type='diagonal',trend=[0])
    mod = sm.tsa.VARMAX(y.T, order=(0,1),trend=[0])
    res = mod.fit(start_params=np.concatenate((Theta.ravel('C'),np.ones(N))), maxiter=100, disp=True)
    res = mod.fit(maxiter=100, disp=True)
    res.VARMAXResults()
    res.summary()
    mod.res_ar
    res.params
    res.coefficient_matrices_vma
    res.model._params_ma
    res.smoother_results
    Theta
    ...
    plt.plot(y[1],color='black',lw=0.6)
    plt.plot(eps[1,-T:],color = 'red',lw=0.6)
    plt.show()
    
    tensor_op.unfold(a['G'],0).numpy()
    
    np.outer (A[:,0],np.linalg.inv(A)[0,:])
    np.linalg.det(A) 
    import matplotlib.pyplot as plt
    y.shape
    plt.plot(y[0,:])
